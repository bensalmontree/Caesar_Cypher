...
def int_check(question):
    ...
    while not valid:
        ...
        # Continue if response is an integer
        if response.isnumeric() == True:
            return response

        # If left blank and encrypting, randomly generate key
        elif response == "" and get_cipher == "encrypt":
            random_number = random.randint(1,25)
            print("Shift value is left blank, randomly generated shift: " + str(random_number))
            return random_number 

        # If left blank and decrypting, iterate through all decryption keys
        elif response == "" and get_cipher == "decrypt":
            hack = True
